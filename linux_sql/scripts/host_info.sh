...
# Checking to make sure that the correct number of parameters were passed to the script.
if [ "$#" != "5" ];
then
  echo "Incorrect number of parameters provided. Please use the format `host_info.sh [psql_host] [psql_port] [database_name] [psql_user] [psql_password]`, replacing [field] with the appropriate information."
fi
